...
module.exports = async function () {
  ...
  for (let i = 0; i < rss.items.length; i++) {
    ...
    if (obj.rating) {
      obj.ratingAve =
        (parseFloat(obj.ratingJoe) + parseFloat(obj.ratingMike)) / 2;
    }
  ...
  function starify(rating) {
    ...
    var starryString;
    starryString = '<i class="fa-solid fa-star"></i>'.repeat(
      Math.floor(rating)
    );
    if (rating.slice(-2) == ".5") {
  ...
  function fiveStarStr(episodes) {
    ...
    if (fiveStarAgo == 0) {
      fiveStarAgo = "last episode.";
    } else {
      fiveStarAgo = `${fiveStarAgo} episodes ago.`;
    }
    return `${hs}Golden Books${he}: Out of ${ratingEps} ratings, ${hs}${starEps} books${he} have been given a ${hs}double five stars${he}. That's ${Math.floor(
      (starEps / ratingEps) * 100
    )}%, or every ${
      hs + Math.round(ratingEps / starEps)
    }th book ${he}on average!<br><br>The most ${hs}recent book${he} to be awarded full stars by both Mike and Joe was ${hs}<a href="${
      fiveStarBook.link
    }">${fiveStarBook.no + ": " + fiveStarBook.title}</a>${he}, ${fiveStarAgo}`;
  }
  ...
  function biggestDiff(episodes) {
    ...
    var hs = "<span class='card-hl'>";
    var he = "</span>";
    return `The ${hs}biggest difference${he} in rating is ${
      hs + maxDiff
    } stars${he} on episode ${maxObj.no}: <a href="${maxObj.link}">${
      maxObj.title
    }</a>. Mike gave ${hs + maxObj.ratingMike} stars ${he} and Joe gave ${
      hs + maxObj.ratingJoe
    } stars${he}.<br><br>Both gave the ${hs}same rating${he} on ${
      hs + samesies
    } books${he}. That's ${Math.round(
      (samesies / epArr.length) * 100
    )}% of books rated.`;
  }
  function getDate() {
    var d = new Date();
